...
- The base plugin contains `UnrealAI`, `UnrealAIAccess`, and `UnrealAITransport`; optional auth and experimental integrations live under `Addons/`. Keep the base independent of those addons and AutonomousAgents. `UnrealAI` is a runtime module. Do not introduce editor-only dependencies into its runtime or public API.
