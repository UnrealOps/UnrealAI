...
namespace
{
const FName AnthropicStrategyProviderName(TEXT("anthropic.messages"));

class FAnthropicContinuationCommit final : public IUnrealAINativeSseContinuationCommit
{
  public:
	explicit FAnthropicContinuationCommit(FUnrealAIAnthropicMessagesContinuationCommit &&InCommit)
...
class FAnthropicDecoder final : public IUnrealAINativeSseDecoder
{
  public:
	FAnthropicDecoder(const FUnrealAIModelRequest &Request, IUnrealAINativeSseProtocol::FEventSink Sink,
  ...
  private:
	FUnrealAIAnthropicMessagesStreamDecoder Decoder;
};

class FAnthropicProtocolStrategy final : public IUnrealAINativeSseProtocol
{
  public:
	FName GetProviderName() const override
	{
		return AnthropicStrategyProviderName;
	}

	int32 GetMaximumRequestBodyBytes() const override
	{
		return FUnrealAIAnthropicMessagesWireRequest::MaxBodyBytes;
	}

	int64 GetMaximumStreamBytes() const override
	{
		return FUnrealAIAnthropicMessagesStreamDecoder::MaxStreamBytes;
	}
					  ...
					  FUnrealAIModelError &OutError) const override
	{
		...
		FUnrealAIAnthropicMessagesBuildContext Context;
		Context.Destination = Destination;
		FUnrealAIAnthropicMessagesWireRequest Wire;
		FUnrealAIAnthropicMessagesContinuationCommit Commit;
		if (!FUnrealAIAnthropicMessagesRequestBuilder::Stage(Request, Context, Wire, Commit, OutError))
		{
			return false;
		}
		OutBodyUtf8 = MoveTemp(Wire.BodyUtf8);
		OutContinuationCommit = MakeUnique<FAnthropicContinuationCommit>(MoveTemp(Commit));
		return true;
	}
	...
	{
		return MakeUnique<FAnthropicDecoder>(Request, MoveTemp(Sink), MoveTemp(IgnoredEventObserver));
	}

  private:
};

FUnrealAINativeSseModelProviderConfig MakeNativeConfig(const FUnrealAIAnthropicMessagesProviderConfig &Config)
{
	FUnrealAINativeSseModelProviderConfig Native;
	Native.ProviderName = AnthropicStrategyProviderName;
	Native.Connection = Config.Connection;
	Native.RelativePath = TEXT("/v1/messages");
	Native.CredentialPresentation = EUnrealAIHttpCredentialPresentation::AnthropicApiKeyHeader;
	Native.FixedHeaders.Add({TEXT("anthropic-version"), TEXT("2023-06-01")});
	Native.ModelProfiles = Config.ModelProfiles;
	Native.Protocol = MakeShared<FAnthropicProtocolStrategy, ESPMode::ThreadSafe>();
	Native.ErrorCodePrefix = TEXT("anthropic");
...
FUnrealAIAnthropicMessagesProviderConfig::MakeMessagesModelProfile(const FString &ModelId)
{
	...
	Profile.MaximumOutputTokens = FUnrealAIModelRequest::MaxOutputTokensLimit;
...
bool FUnrealAIAnthropicMessagesProviderConfig::ValidateShape(FString &OutError) const
{
	OutError.Reset();
	if (ModelProfiles.IsEmpty() || ModelProfiles.Num() > MaxModelProfiles)
	{
		OutError = TEXT("Anthropic Messages provider requires an exact bounded model allowlist.");
		return false;
	}
	return MakeNativeConfig(*this).ValidateShape(OutError);
}

FUnrealAIAnthropicMessagesProvider::FUnrealAIAnthropicMessagesProvider(
...
FName FUnrealAIAnthropicMessagesProvider::GetProviderName() const
{
	return Implementation->GetProviderName();
}

FUnrealAIModelProviderDescriptor FUnrealAIAnthropicMessagesProvider::Describe() const
{
	return Implementation->Describe();
}

EUnrealAIModelCapability FUnrealAIAnthropicMessagesProvider::GetCapabilities(const FString &ModelId) const
{
	return Implementation->GetCapabilities(ModelId);
}

FUnrealAIModelProfileProjection FUnrealAIAnthropicMessagesProvider::GetModelProjection(const FString &ModelId) const
{
	return Implementation->GetModelProjection(ModelId);
}

bool FUnrealAIAnthropicMessagesProvider::StartRequest(
...
void FUnrealAIAnthropicMessagesProvider::BeginShutdown()
{
