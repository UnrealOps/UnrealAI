...
The first generated release is bootstrapped as `v0.1.0`. Before `1.0.0`, a breaking Conventional Commit increments the minor version; features increment minor, and fixes increment patch. Documentation and maintenance commits do not create releases or appear in the generated changelog by default.

The workflow requests only `contents: write`, `issues: write`, and `pull-requests: write`, uses a concurrency lock, and pins third-party actions to complete commit SHAs. Candidate branches are treated as data: version synchronization and validation execute scripts loaded from trusted `main`, not code from the generated branch. In **Settings → Actions → General**, allow GitHub Actions to create pull requests. The built-in `GITHUB_TOKEN` is sufficient for release management. GitHub creates workflow runs for pull requests opened or updated with that token in an approval-required state. If branch protection requires checks on the generated release pull request to start automatically, configure a fine-grained `RELEASE_PLEASE_TOKEN` secret with repository contents, issues, and pull-request access; the workflow uses it when present and otherwise falls back to `GITHUB_TOKEN`.
...
For local ushell validation, prepare a fresh consumer with `Scripts/ci/prepare_native_consumer.py --output <fresh-directory> --mode <base|auth|experimental> --engine-association <association-from-your-project>`, select its `.uproject`, then run `.build editor` and `.run editor -- -CmdLineFile=<consumer>/Automation.txt` with the usual unattended options. Use an absolute command-file path ending in `.txt`. A source engine needs a valid EngineAssociation to select a project outside its source tree.

`Scripts/ci/package_with_dependencies.py --engine <engine> --plugin <plugin-directory> --dependency <dependency-directory> --output <fresh-directory> --platform <Mac|Win64|Linux>` writes a `PackageCommand.json` argument array. Run that command through `.uat` in ushell. Default packaging compiles Editor, Development Game, and Shipping Game. The explicit `--editor-only --editor-modules` development option disables PCH/unity in staged plugin rules without forcing a complete source-engine rebuild; this qualifies Editor plugin includes only and is not a release/client/server qualification result.

Native reports must state which configuration was actually exercised. A prepared command, successful portable check, or Editor-only package must never be reported as a full native release gate.
