...
bool TryBuildResponsesPolicy(FUnrealAIOpenAIResponsesProviderConfig &OutConfig, FString &OutError)
{
	...
	// This subscription endpoint uses the Codex request subset, without max_output_tokens.
	OutConfig.bSendMaxOutputTokens = false;
	// Successful responses on this route may omit Content-Type; the strict SSE decoder still validates every event.
	OutConfig.bAllowMissingResponseContentType = true;
	// This route streams full items once, then sends a metadata-only response.completed event.
	OutConfig.bAllowEmptyTerminalOutput = true;
	...
	OutConfig.FixedHeaders.Add({TEXT("User-Agent"), TEXT("codex_cli_rs/0.0.0 (UnrealAI)")});
	OutConfig.ModelProfiles.Add(OutConfig.MakeToolCapableModelProfile(TEXT("gpt-5.5")));
